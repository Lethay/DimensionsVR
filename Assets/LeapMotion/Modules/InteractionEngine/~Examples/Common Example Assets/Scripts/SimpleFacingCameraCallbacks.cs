...
namespace Leap.Unity.Examples {
  ...
  public class SimpleFacingCameraCallbacks : MonoBehaviour {
    ...
    [HideInInspector]
    public bool leapHandsDetected = false;
    ...
    void Update() {
      ...
      if (!leapHandsDetected){
          if (handModel != null) { 
            if(handModel.IsTracked) leapHandsDetected = true;
            else return; //prevent menu appearing at the beginning of the scene and confusing users
          }
          else{
            Debug.Log("SimpleFacingCameraCallbacks: handModel is null.");
            return;
          }
      }
